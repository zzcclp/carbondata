...
class CarbonCommandSuite extends Spark2QueryTest with BeforeAndAfterAll {
  ...
  test("separate visible and invisible segments info into two files") {
    ...
    var detail = SegmentStatusManager.readLoadMetadata(carbonTable.getMetaDataFilepath)
    var historyDetail = SegmentStatusManager.readLoadHistoryMetadata(carbonTable.getMetaDataFilepath)
